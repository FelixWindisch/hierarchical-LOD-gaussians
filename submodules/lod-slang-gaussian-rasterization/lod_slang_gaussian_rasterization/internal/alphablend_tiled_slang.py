...
import torch
from lod_slang_gaussian_rasterization.internal.render_grid import RenderGrid
import lod_slang_gaussian_rasterization.internal.slang.slang_modules as slang_modules
from lod_slang_gaussian_rasterization.internal.tile_shader_slang import vertex_and_tile_shader
...
def render_alpha_blend_tiles_slang_raw(xyz_ws, rotations, scales, opacity, 
                                       sh_coeffs, distance_mu, distance_sigma, distances, active_sh,
    ...
    image_rgb = AlphaBlendTiledRender.apply(
        ...
        inv_cov_vs,
        opacity,
        rgb, distance_mu, distance_sigma, distances,
        render_grid)
...
class AlphaBlendTiledRender(torch.autograd.Function):
    ...
    def forward(ctx, 
                ...
                xyz_vs, inv_cov_vs, opacity, rgb, distance_mu, distance_sigma, distances, render_grid, device="cuda"):
        ...
        splat_kernel_with_args = alpha_blend_tile_shader.splat_tiled(
            ...
            opacity=opacity, rgb=rgb, distance_mu=distance_mu, distance_sigma=distance_sigma, distance=distances,
        ...
        ctx.save_for_backward(sorted_gauss_idx, tile_ranges,
                              xyz_vs, inv_cov_vs, opacity, rgb, distance_mu, distance_sigma, distances, 
    ...
    def backward(ctx, grad_output_img):
        ...
        distance_mu_grad = torch.zeros_like(distance_mu)
        distance_sigma_grad = torch.zeros_like(distance_sigma)
        #breakpoint()
        ...
        kernel_with_args = alpha_blend_tile_shader.splat_tiled.bwd(
            ...
            rgb=(rgb, rgb_grad),
            distance_mu=(distance_mu, distance_mu_grad),
            distance_sigma=(distance_sigma, distance_sigma_grad),
            distance=distances,
        ...
        return None, None, xyz_vs_grad, inv_cov_vs_grad, opacity_grad, rgb_grad, distance_mu_grad, distance_sigma_grad, None, None
