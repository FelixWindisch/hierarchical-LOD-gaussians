...
#Standard
densify_interval = 5000 #5000
densify_radii = False
Densify_Percent = 1.03
lr_multiplier = 1
#Unused
...
def training(dataset, opt:OptimizationParams, pipe, saving_iterations, checkpoint_iterations, checkpoint, debug_from, cons_graph):
    global Cache_Size
    global Cache_Size_After_Reduction
    ...
    if checkpoint:
        checkpoint_file = os.path.join(scene.model_path, f"chkpnt{checkpoint_iterations[0]}.pth")
    ...
    while iteration < opt.iterations + 1:
        for viewpoint_batch in training_generator:
            for viewpoint_cam in viewpoint_batch:
                if Use_Consistency_Graph:
                    current_camera_index = int(consistency_graph.random_walk_node(cons_graph, (str(current_camera_index))))
...
#
                ...
                if iteration % Clear_Cache_Iterations == 0:
                    temp1 = Cache_Size
                    temp2 = Cache_Size_After_Reduction
                    Cache_Size = 0
                    Cache_Size_After_Reduction = 0
                ...
                if iteration % Clear_Cache_Iterations == 0:
                    Cache_Size = temp1
                    Cache_Size_After_Reduction = temp2
                #if iteration % 1000 == 0:
