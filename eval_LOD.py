...
def render(dataset, opt:OptimizationParams, pipe, saving_iterations, checkpoint_iterations, checkpoint, debug_from,  hierarchy_path, replay=False, cam_path_id=0, test_set=False):
    ...
    random_image = random.randrange(0, len(scene.getTrainCameras()), 1)
    i = 0
    train_camera_data_set = scene.getTrainCameras()
    PSNR = []
    Gaussians = []
    for x in [0]:
        for random_image in [random.randrange(0, len(scene.getTrainCameras()), 1) for i in range(5)]:
            for distance_multiplier in torch.range(1, 20, 0.4):
                i+=1
                
                
                viewpoint_cam = train_camera_data_set[int(random_image)]
